...
decryptInput keysVar ZKProveRequest{..} = do
    ...
    json <- stripPKCS7 paddedText

    -- Lastly, parse the payload into JSON
    case decode $ BS.fromStrict json of
        Nothing ->
            throw
                (ZKPEDecryptionError $ ZKDecryptionFailed $ "JSON decoding failed. Attempted to decode " <> (T.pack . show) json)
